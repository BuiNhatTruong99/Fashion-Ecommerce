...
import { getProducts } from '@/apis/endPoints/products.endpoint';
...
const ProductList = ({ categoryId, limit }: IProductListProps) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      const response = await getProducts(categoryId, limit);
  ...
  return (
    <div className="mt-12 flex gap-x-8 gap-y-16 justify-between flex-wrap">
      {products.map((product) => (
        <Link href="/product" key={product.id} className="w-full flex flex-col gap-4 sm:w-[45%] lg:w-[22%]">
